...
router.get('/analytics', async (req, res) => {
  try {
    ...
    const [analytics, retention, channelCohorts, calendarConnection, acquisition, inviteLoop, appleAdsUsers, referrals, signupSources, pendingReplyLeaks, paywallFunnel, onboardingFunnel, meterStats] = await Promise.all([
      ...
      db.getOnboardingFunnel({ days: Math.min(days, 30) }).catch(() => null),
      db.getMeterStats().catch(() => null),
    ]);
    ...
    return res.json({
      ...analytics, retention, channelCohorts, calendarConnection, acquisition, inviteLoop, appleAds, referrals, signupSources, pendingReplyLeaks, paywallFunnel, onboardingFunnel, meterStats,
    });
  } catch (err) {
