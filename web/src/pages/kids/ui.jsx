...
export function StarPill({ n, big }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: big ? 7 : 4, background: '#FFF3D6', padding: big ? '8px 16px' : '3px 10px 3px 8px', borderRadius: 999, border: '2px solid #FBE0A0' }}>
      <Star s={big ? 24 : 15} />
      <b style={{ fontSize: big ? 24 : 14, color: '#B77B10', fontWeight: 700 }}>{n}</b>
    </span>
  );
}
  ...
  return (
    <div onClick={onDone} style={{ position: 'fixed', inset: 0, zIndex: 70, background: 'rgba(49,43,75,0.5)', backdropFilter: 'blur(3px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24 }}>
      ...
      <div className="kids-anim" style={{ background: '#fff', borderRadius: 36, padding: '34px 30px', textAlign: 'center', width: '100%', maxWidth: 320, boxShadow: '0 20px 60px rgba(49,43,75,0.4)', animation: 'kids-burst-pop .5s ease forwards' }}>
        ...
        <div style={{ fontSize: 26, fontWeight: 700, marginTop: 8, color: KIDS_INK.ink }}>{data.title}</div>
...
// The Housemait house mark filled with the kid's gradient (white windows)
// plus the yellow KIDS badge — per the spec's adapted logo.
export function KidsLogo({ c1, c2, size = 30, showWord = true }) {
  const gid = 'klg_' + String(c1 || '').replace('#', '');
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
      <svg width={size} height={size} viewBox="16 -2 205 168">
        ...
        <path fill="#fff" d="M68.91,88.98c-12.99-3.42-15.25-21.12-3.81-27.97,10.83-6.74,25.17,2.44,23.15,15.15-1.08,8.84-10.55,15.28-19.15,12.86l-.19-.05Z" />
        <path fill="#fff" d="M125.88,89c-12.51-2.66-16.19-19.3-6.07-27.06,9.59-7.72,24.58-1.08,25.02,11.28.67,9.77-9.22,18.12-18.75,15.82l-.19-.04Z" />
      </svg>
      {showWord && <span style={{ fontSize: size * 0.6, fontWeight: 700, letterSpacing: -0.5, color: KIDS_INK.ink }}>Housemait</span>}
      <span style={{ fontSize: Math.max(10, size * 0.3), fontWeight: 700, letterSpacing: 1, color: '#fff', background: KIDS_INK.sun, padding: '3px 8px', borderRadius: 999, boxShadow: '0 3px 0 rgba(233,154,18,0.35)' }}>KIDS</span>
    </div>
  );
}
